...
\section{Implementation}
  ...
  placement, a function is used, $f_{fitness}$ that takes as its argument disk
  stats and returns a positive number henceforth known as a fitness value.
  ...
  simulations to compare each algorithm at different orders of magnitude.
    ...
    but will not be a good candidate if it's desired for the WeightedVector to
    select objects with probability proportional to its weight.
  ...
  Table \ref{tab:wvpub} shows the WeightedVector public interface.
    ...
    Suppose that a call to Sample() on the WeightedVector returns $D$. The
    ...
    in the WeightedVector with the expected value and the difference is
