...
\section{Introduction}
  ...
  connected in a full mesh network. An example full mesh network topology is
  shown in Figure \ref{fig:adsf-network}. This full mesh configuration allows
  for the clustering of virtual machines (VMs) that provide file system
  services by allowing each VM to send messages to each other VM.
  ...
  variables could have on a write, the file system should take into account how
  ...
  \subsection{Nutanix and the Acropolis Base System} \label{sssec:abs}
    ...
    distributed manner. It is also the distributed database used by Arithmos
    to store cluster statistics. The version of Cassandra running in NDFS is a
    heavily modified Apache Cassandra \cite{cassandra}. One of the main
    ...
    data or whether to write the data to a new extent group. In-place overwrites are similar to
    ...
    replicas from the preferred nodes. Since replicas must reside on the same
...
\section{Prior Work}
  ...
  estimate the finish time of tasks and schedule the work that is expected to
  take the longest to complete first in the pool of available nodes.
...
\section{Implementation}
  ...
  queue length over some stretch of time, $t_{q}$, and one fitness function of
  ...
  Increasing $t_{q}$ from 0 towards $q_{ceiling}$ decreases the fitness value,
...
together (Equation \ref{eqn:multiplicative-fitness}).
  ...
  average SSD utilization percentage of the nodes running a workload, $u_{w}$,
...
\FloatBarrier
 \begin{thebibliography}{99}
  ...
  \bibitem{azar1994} Azar, Y., Broder, A., Karlin, A., and Upfal, E. Balanced allocations. In Proceedings of the 26th ACM Symposium on the Theory of Computing, pages 593\{602, 1994.
  ...
  \bibitem{igrp} Hedrick, C., and Bosack, L. (1991). An introduction to IGRP. Rutgers-The State University of New Jersey Technical Publication, Laboratory for Computer Science.
  ...
  \bibitem{xie2010} Xie, J., Yin, S., Ruan, X., Ding, Z., Tian, Y., Majors, J., Manzanares, A., and Qin, X. (2010, April). Improving mapreduce performance through data placement in heterogeneous hadoop clusters. In Parallel and Distributed Processing, Workshops and Phd Forum (IPDPSW), 2010 IEEE International Symposium on (pp. 1-9). IEEE.
