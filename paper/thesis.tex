...
in the Nutanix distributed file system which will remedies some common problems
...
\section{Implementation}
  ...
  way that we call a single function to return a disk ID from a sampling set
  using a weighted random sampling algorithm that uses the disks' fitness
  values. An object container class is needed similar to the data structures in
  the C++ Standard Template Library \cite{stl}. A requirement for this object
  is that must be able to access elements of arbitrary types via arbitrary
  weighted random sampling methods. The \texttt{WeightedVector} class was
  implemented to fulfill this requirement. 
  ...
  \FloatBarrier
    ...
    vectors accompanying an inner\_vector\_ of objects as shown in Table
    \ref{tbl:inner-vec-1}.
    ...
    $D$. The only way to restore sampling state is to call \texttt{Reset()}. A
...
\section{User Guide}
  ...
  filled via workload generation on each node. Each of the other nodes then
  clone the worker VM so that all nodes in the cluster have an identical VM and
  tier utilization.
  ...
  recreate the Nutanix cluster via \texttt{cluster -s <cvm IPs> create}. The
  process is then repeated for each test.
